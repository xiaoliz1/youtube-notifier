...
# 1x1 透明像素（作为 document 发送，避免文件太大）
...
def get_latest_videos(channel_id):
    try:
        ...
        thumb = e.media_thumbnail[0]['url'] if e.get('media_thumbnail') else None
        return [{
            'title': title,
            'link': e.link,
...
def send_telegram_notification(video, channel_name):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID or not video['thumbnail']:
        return
    ...
    try:
        # 下载封面图
        img_data = requests.get(video['thumbnail'], timeout=10).content

        files = {
            'thumb': ('thumb.jpg', img_data, 'image/jpeg'),
